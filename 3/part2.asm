...
; searches for "gear ratios" in `lineCurr`
; gear ratios found are added to [sum]
;
...
_searchCntPN_nextChar:
        ...
        mov sil, byte [r13]             ; read current char. Is it a gear
        cmp sil, '*'                    ; C is gear <-> C == '*'
        jne _searchCntPN_nextChar

_searchCntPN_partFound:
        ; at this point we know that lineCurr[rbx] has a "gear":
...
_searchCntPN_bottom:
        ...
        ; r14 contains how many results to check. It's 8 because of
        ; (left, mid, right) * (top, bottom) + (left, right)
        mov r13, 0                      ; used to store last search result
        mov r14, 8                      ; how many results to check
        mov r15, 0                      ; how many distinct PNs are there
        mov r12, 1                      ; temporary value to be added to [sum]
_searchCntPN_checkResults:              ; count PNs, remove duplicates
        sub r14, 1
        js _searchCntPN_postGear

        pop rdi                         ; restore last search result
        cmp rdi, 0
        je _searchCntPN_checkResults    ; skip if 0 (nothing found)
        cmp edi, r13d                   ; R13D = R13 dword
        je _searchCntPN_checkResults    ; skip if found same number as last time
...
_searchCntPN_postGear:                  ; add gear ration if exactly 2 PNs found
        cmp r15, 2
        jne _searchCntPN_nextChar

        add qword [sum], r12
        jmp _searchCntPN_nextChar

_searchCntPN_ret:
        mov rax, 0

        pop r15                         ; restore CSRs
        pop r14
        pop r13
        pop r12
        pop rbx
